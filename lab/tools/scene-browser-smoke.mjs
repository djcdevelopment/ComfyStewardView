...
const cases = [
  { name:'pilot-862', pieces:862, families:13, coverage:{real:862,estimated:0,unknown:0},
  ...
  ...(!pilotOnly ? [{ name:'stress-22387', pieces:22387, families:13,
    coverage:{real:22199,estimated:0,unknown:188},
...
try {
  ...
  for (const sceneCase of cases) {
    ...
    const controlResult = await page.call('Runtime.evaluate',{
      expression:`(() => {
        ...
        return {surface,before,hidden,restored,fly,reset:window.__stewardSceneReceipt.cameraMode};
      })()`,
      returnByValue:true
    });
    ...
    if (receipt.families?.length !== sceneCase.families) failures.push(`families ${receipt.families?.length} != ${sceneCase.families}`);
    ...
    if (receipt.frameP95Ms > p95Limit) failures.push(`p95 ${receipt.frameP95Ms}ms > ${p95Limit}ms`);
  ...
  const result = {
    ...
    baseUrl:base.href, limits:{startupMs:startupLimit,frameP95Ms:p95Limit}, receipts
  };
