...
if (exercise) {
  ...
  const drag = async (x1,y1,x2,y2) => {
    ...
    await cdp('Input.dispatchMouseEvent', { type:'mouseMoved', x:x2, y:y2, button:'left', buttons:1 });
    await cdp('Input.dispatchMouseEvent', { type:'mouseReleased', x:x2, y:y2, button:'left', buttons:0, clickCount:1 });
  };
  await drag(r.x+r.w*.37, r.y+r.h*.3, r.x+r.w*.66, r.y+r.h*.72);
  await new Promise(resolve => setTimeout(resolve, 1800));
  await drag(r.x+r.w*.38, r.y+r.h*.32, r.x+r.w*.68, r.y+r.h*.72);
  await new Promise(resolve => setTimeout(resolve, 2200));
  await drag(r.x+r.w*.38, r.y+r.h*.32, r.x+r.w*.68, r.y+r.h*.72);
  await new Promise(resolve => setTimeout(resolve, 2200));
  await drag(r.x+r.w*.34, r.y+r.h*.28, r.x+r.w*.72, r.y+r.h*.76);
  await new Promise(resolve => setTimeout(resolve, 2800));
  await drag(r.x+r.w*.34, r.y+r.h*.28, r.x+r.w*.72, r.y+r.h*.76);
  await new Promise(resolve => setTimeout(resolve, 3200));
  await drag(r.x+r.w*.3, r.y+r.h*.24, r.x+r.w*.76, r.y+r.h*.8);
  await new Promise(resolve => setTimeout(resolve, 3200));
  await drag(r.x+r.w*.3, r.y+r.h*.24, r.x+r.w*.76, r.y+r.h*.8);
  await new Promise(resolve => setTimeout(resolve, 3200));
...
console.log(JSON.stringify({ targetUrl, output, state, errors }, null, 2));
...
if (errors.length || !state.legendVisible || /NO RASTER|Preparing/.test(`${state.status} ${state.title}`) ||
    (exercise && (!state.inspectorVisible || !/Birch trees/.test(state.status) || state.exactState === 'RASTER'))) process.exitCode = 1;
