...
const PROFILE_PIECE_BANDS = [
  {key: 'lt100', label: '<100'}, {key: '100-499', label: '100–499'},
  {key: '500-999', label: '500–999'}, {key: '1000-4999', label: '1,000–4,999'},
  {key: '5000+', label: '5,000+'}, {key: 'unknown', label: 'Unmeasured'},
];

function profilePieceBand(pieces) {
  if (pieces == null || !Number.isFinite(Number(pieces))) return 'unknown';
...
function profileBuildInventory(threadDoc) {
  const self = threadDoc?.builderKey;
  const cells = new Map();
  const eras = (threadDoc?.eras || []).map((block) => Number(block.era)).sort((a, b) => b - a);
  for (const block of threadDoc?.eras || []) for (const album of block.albums || []) {
    const mine = (album.contributors || []).find((item) => item?.builderKey === self)?.pieces;
    const band = profilePieceBand(mine);
    const key = `${block.era}:${band}`;
    const cell = cells.get(key) || {era: Number(block.era), band, builds: 0, photographed: 0};
    cell.builds += 1;
    if (album.photos?.length) cell.photographed += 1;
    cells.set(key, cell);
  }
  return {eras, bands: PROFILE_PIECE_BANDS, cells};
}

function profileBrowseRows(threadDoc, {era = 'all', band = 'all', query = '', sort = 'mine'} = {}) {
  if (!threadDoc) return [];
  const self = threadDoc.builderKey;
  ...
  const scored = (threadDoc.eras || []).flatMap((block) => block.albums || [])
    ...
    .map((album) => ({album, mine: (album.contributors || [])
      .find((c) => c && c.builderKey === self)?.pieces ?? null}))
    .filter((row) => band === 'all' || profilePieceBand(row.mine) === band)
    .map((row) => ({...row, mine: row.mine ?? -1}));
  const compare = (a, b) => {
...
// Keep the opening shortlist photographic without hiding the largest recorded work
// that has not been photographed. Each lane retains the requested piece ranking;
// explicit sort choices bypass this guide and show a strict top-N instead.
function profileGuidedPicks(albums, limit = 6) {
  const pictured = albums.filter((album) => album.photos?.length);
  const unpictured = albums.filter((album) => !album.photos?.length);
  const picks = [...pictured.slice(0, Math.ceil(limit / 2)), ...unpictured.slice(0, Math.floor(limit / 2))];
  const chosen = new Set(picks.map((album) => album.buildKey));
  for (const album of albums) {
    if (picks.length >= limit) break;
    if (!chosen.has(album.buildKey)) { picks.push(album); chosen.add(album.buildKey); }
  }
  return picks;
}
...
// Fixed bins keep the atlas comparable between a small one-era profile and a
// megabuilder's thread. Counts, not hue, are the primary evidence on each tile.
function profileEraAtlas(threadDoc) {
  const blocks = threadDoc?.eras || [];
  if (!blocks.length) return [];
  const byEra = new Map(blocks.map((block) => [block.era, block]));
  const numbers = [...byEra.keys()];
  const range = Array.from({length: Math.max(...numbers) - Math.min(...numbers) + 1}, (_, i) => Math.min(...numbers) + i);
  return range.map((era) => {
    const block = byEra.get(era);
    const albums = block?.albums || [];
    const photos = albums.reduce((sum, album) => sum + (album.photos?.length || 0), 0);
    const photographed = albums.filter((album) => album.photos?.length).length;
    const count = albums.length;
    const density = count === 0 ? 0 : count < 10 ? 1 : count < 50 ? 2 : count < 200 ? 3 : 4;
    return {era, albums: count, photographed, photos, density};
  });
}

function profileCoBuilderSummary(threadDoc, era = 'all') {
  if (!threadDoc) return {total: 0, ranked: []};
...
if (typeof module !== 'undefined') {
  module.exports = {
    ...
    portraitIndex, eraBounds, heroAliases, pickMosaicAlbums, profileBrowseRows, profileBrowsePage, profileEraAtlas, profileCoBuilderSummary, distinctAttributions,
    PROFILE_PIECE_BANDS, profilePieceBand, profileBuildInventory, profileGuidedPicks,
...
const initCreatorsPage = async () => {
  ...
  let directory = null;
  let thread = null;
  const profileBrowse = {era: 'all', band: 'all', query: '', sort: 'guided', workPage: 0, buildPage: 0, ledgerOpen: false};
  let browseAlbums = [];
  let selectedBuildKey = null;
  let eraAtlas = [];
  let buildInventory = null;
  let coBuilderSummaries = new Map();
  ...
  function hydrateCredits() {
    for (const anchor of document.querySelectorAll('a.credit[data-builder-key], a.resident[data-builder-key], .top8-chip .top8-name[data-builder-key], .kin-tree .kin-node-name[data-builder-key], .kin-profile-card h3[data-builder-key]')) {
  ...
  function top8Chip(entry, rank) {
    ...
    chip.onclick = () => {
      ...
      const active = ensurePair(entry.builderKey);
  ...
  function renderTopEight({total: coBuilders = 0, ranked = []} = {}) {
    ...
    if (initialPair.kin && !ranked.some((entry) => entry.builderKey === initialPair.kin)) {
      const scoped = profileBrowse.era === 'all' ? thread : {...thread, eras: thread.eras.filter((block) => String(block.era) === profileBrowse.era)};
      const extra = computeTopEight(scoped, Infinity).find((entry) => entry.builderKey === initialPair.kin);
    ...
    note.append(kinshipLink, ' for every co-builder');
    panel.append(note);
    return panel;
  }

  // A small, era-aware summary on the profile. The complete branch drawing stays on
  // the dedicated kinship page; a pair ledger is mounted only after a selection.
  function renderKinshipEmbed() {
    const summary = coBuilderSummaries.get('all') || {total: 0, ranked: []};
    const compact = node('section', null, 'kin-beside');
    compact.id = 'kin-beside';
    compact.setAttribute('aria-labelledby', 'kin-beside-h2');
    const title = node('h2', 'Who they built beside');
    title.id = 'kin-beside-h2';
    compact.append(title, node('p', `${plural(summary.total, 'co-builder')} have qualifying saved-piece credits on some of the same builds. That is a recorded overlap, not an inferred friendship. Start with one person to see the shared builds.`, 'kin-beside-sub muted'));
    if (summary.ranked.length) {
      ...
      const cards = node('div', null, 'kin-profile-cards');
      for (const entry of featured) {
        const card = node('article', null, entry.builderKey === initialPair.kin ? 'kin-profile-card kin-profile-linked' : 'kin-profile-card');
        card.append(ribbonPortrait(entry.builderKey));
        const text = node('div', null, 'kin-profile-card-copy');
        const name = node('h3', buildersByKey.get(entry.builderKey)?.displayName || placeholderName(entry.builderKey));
        name.dataset.builderKey = entry.builderKey;
        text.append(name);
        const eras = thread.eras.filter((block) => block.albums.some((album) => qualifyingSharedCredit(album, thread.builderKey, entry.builderKey))).map((block) => block.era).sort((a, b) => a - b);
        const eraLabel = eras.length === 1 ? `era ${eras[0]}` : `${eras.length} eras (${eras[0]}–${eras.at(-1)})`;
        text.append(node('p', `${plural(entry.sharedAlbums, 'shared build')} · ${entry.sharedPieces.toLocaleString()} overlapping credited pieces · ${eraLabel}`, 'kin-profile-card-facts'));
        const href = new URL(`kinship/?builder=${thread.builderKey}&kin=${entry.builderKey}`, base);
        text.append(link('Explore shared builds', href));
        card.append(text);
        cards.append(card);
      }
      compact.append(cards);
    }
  ...
  function renderWorkCarousel() {
    ...
    workAlbums = profileBrowseRows(thread).filter((album) => Array.isArray(album.photos) && album.photos.length);
    ...
    section.append(node('p', 'A photo tour of the builds recorded on this profile. Browse the images here; era and build filters below do not change this tour.', 'work-orientation muted'));
    ...
    section.append(node('p', `${plural(shot, 'photographed build')} on this profile`, 'mosaic-foot muted'));
    section.append(link('Explore your work era by era', '#era-overview'));
    return section;
  }
  ...
  function refreshAlbumCard(album, targetBuilderKey) {
    const focused = $('build-focus-host')?.querySelector(`article.album[data-build-key="${album.buildKey}"]`);
    if (focused) {
      $('build-focus-host').replaceChildren(renderFocusedBuild(album));
    }
    if ($('work-stage')?.querySelector(`article.album[data-build-key="${album.buildKey}"]`)) paintWorkStage();
    renderTagChips(externalParticipation);
  }
  ...
  function renderHeroCard() {
    ...
    text.append(...ordered);
    let orientation = $('hero-orientation');
    if (!orientation) {
      orientation = node('p', 'This page follows the pieces credited to you in saved builds. Photographs show whole builds; some recorded work has not been photographed yet.', 'hero-orientation');
      orientation.id = 'hero-orientation';
    }
    let journey = $('hero-journey');
    if (!journey) {
      journey = node('nav', null, 'hero-journey');
      journey.id = 'hero-journey';
      journey.setAttribute('aria-label', 'Explore this builder profile');
      if (thread.eras.some((block) => block.albums.some((album) => album.photos?.length))) {
        journey.append(link('See photographs', '#work'));
      }
      if (thread.albums) journey.append(link('Explore eras', '#era-overview'), link('Find a build', '#build-browse'));
    }
    text.append(orientation, journey);
    renderHeroDoor();
    hero.hidden = false;
  ...
  function revealAlbum(buildKey, {scroll = true} = {}) {
    const matched = /^[a-f0-9]{64}$/.test(String(buildKey || ''))
      ? thread.eras.flatMap((block) => block.albums).find((item) => item.buildKey === buildKey) : null;
    const newNotice = $('build-browse-notice');
    if (!matched) {
      if (newNotice) { newNotice.textContent = 'This build is not on this builder’s published profile.'; newNotice.hidden = false; }
      return null;
    }
    if (newNotice) newNotice.hidden = true;
    profileBrowse.era = String(matched.era);
    const mine = (matched.contributors || []).find((item) => item?.builderKey === thread.builderKey)?.pieces;
    profileBrowse.band = profilePieceBand(mine);
    profileBrowse.ledgerOpen = false;
    profileBrowse.query = '';
    $('build-era').value = profileBrowse.era;
    $('build-search').value = '';
    browseAlbums = profileBrowseRows(thread, profileBrowse);
    profileBrowse.buildPage = Math.floor(browseAlbums.findIndex((item) => item.buildKey === buildKey) / PAGE_SIZE_EXPLORER);
    selectedBuildKey = buildKey;
    refreshBrowse();
    if (matched.photos?.length) {
      const shot = workAlbums.findIndex((item) => item.buildKey === buildKey);
      if (shot >= 0) {
        workIndex = shot;
        workPhoto = 0;
    ...
    const detail = $('build-focus-host')?.querySelector('article.album');
  ...
  function pairContext(initial = initialPair) {
    return {
      thread,
      ...
      initial: {kin: initial.kin, build: initial.build},
      era: profileBrowse.era,
      revealAlbum,
      base,
  ...
  function ensurePair(key) {
    const host = $('pair-view');
    if (!host || typeof StewardPair === 'undefined') return null;
    if (!pairMounted) {
      StewardPair.mount(host, pairContext({kin: key, build: key === initialPair.kin ? initialPair.build : null}));
      pairMounted = true;
      ...
      if (portraitManifest) StewardPair.update({portraits: portraitManifest});
      return key;
    }
    return StewardPair.select(key);
  }

  function renderEraOverview() {
    const section = node('section', null, 'era-overview');
    section.id = 'era-overview';
    section.setAttribute('aria-labelledby', 'era-overview-h2');
    const heading = node('h2', 'Your work across the eras');
    heading.id = 'era-overview-h2';
    section.append(heading, node('p', workAlbums.length
      ? 'Start with an era, then browse its builds below. Counts follow your saved construction credits; photographed builds have at least one image in the tour above.'
      : 'Start with an era, then browse its saved builds below. The archive has no published photographs for this profile yet.', 'muted'));
    const choices = node('div', null, 'era-overview-choices');
    choices.setAttribute('role', 'group');
    choices.setAttribute('aria-label', 'Choose an era of this builder\'s work');
    const all = node('button', null, 'era-overview-pick');
    all.type = 'button';
    all.dataset.era = 'all';
    all.append(node('strong', 'All eras'), node('span', `${thread.albums.toLocaleString()} builds · ${workAlbums.length.toLocaleString()} photographed`));
    choices.append(all);
    for (const record of [...eraAtlas].reverse()) {
      if (!record.albums) continue;
      const pick = node('button', null, 'era-overview-pick');
      pick.type = 'button';
      pick.dataset.era = String(record.era);
      pick.append(node('strong', `Era ${record.era}`), node('span', `${record.albums.toLocaleString()} builds · ${record.photographed.toLocaleString()} photographed`));
      choices.append(pick);
    }
    for (const pick of choices.querySelectorAll('button')) {
      pick.setAttribute('aria-pressed', String(pick.dataset.era === profileBrowse.era));
      pick.onclick = () => {
        profileBrowse.era = pick.dataset.era;
        profileBrowse.band = 'all';
        profileBrowse.ledgerOpen = false;
        profileBrowse.buildPage = 0;
        selectedBuildKey = null;
        $('build-era').value = profileBrowse.era;
        refreshBrowse();
        $('build-browse')?.scrollIntoView({block: 'start'});
      };
    }
    section.append(choices);
    const detail = node('details', null, 'era-overview-detail');
    detail.append(node('summary', 'Compare credited-piece ranges'), renderBuildMatrix());
    section.append(detail);
    return section;
  }

  function paintEraSelection() {
    for (const pick of document.querySelectorAll('.era-overview-pick')) {
      pick.setAttribute('aria-pressed', String(pick.dataset.era === profileBrowse.era));
    }
  }

  function renderBrowseToolbar() {
    // The era guide above owns navigation. Keep only the tools that help a visitor
    // find a particular build or change the shortlist order, adjacent to results.
    const section = node('div', null, 'build-browse');
    section.id = 'build-browse';
    section.setAttribute('role', 'group');
    section.setAttribute('aria-label', 'Find and order builds');
    const controls = node('div', null, 'build-browse-controls');
    const field = (caption, control, kind) => {
    ...
    const mobileEra = field('Era', era);
    mobileEra.classList.add('build-browse-mobile-era');
    ...
    controls.append(field('Find', search, 'build-browse-search'));

    const sort = node('select');
    sort.id = 'build-sort';
    for (const [value, text] of [['guided', 'Guided mix'], ['mine', 'My pieces first'], ['size', 'Largest build first'], ['newest', 'Newest era first']]) {
      const option = node('option', text);
      option.value = value;
      sort.append(option);
    }
    ...
    controls.append(field('Order', sort, 'build-browse-sort'));
    controls.append(mobileEra);
    ...
    const filters = node('div', null, 'build-active-filters');
    filters.id = 'build-active-filters';
    filters.setAttribute('role', 'group');
    filters.setAttribute('aria-label', 'Active build filters');
    filters.hidden = thread.albums === 0;
  ...
  function paintBrowseFilters() {
    const host = $('build-active-filters');
    if (!host) return;
    host.replaceChildren();
    const add = (label, aria, clear) => {
      const button = node('button', `${label} ×`, 'build-filter-chip');
      button.type = 'button';
    ...
    if (profileBrowse.era !== 'all') add(`Era ${profileBrowse.era}`, 'Remove era filter', () => {
      profileBrowse.era = 'all';
      $('build-era').value = 'all';
    });
    if (profileBrowse.band !== 'all') {
      const band = PROFILE_PIECE_BANDS.find((item) => item.key === profileBrowse.band);
      add(`${band?.label || profileBrowse.band} your pieces`, 'Remove credited-piece range filter', () => { profileBrowse.band = 'all'; });
    }
    if (profileBrowse.query) add(`Find: ${profileBrowse.query}`, 'Remove build search', () => {
      profileBrowse.query = '';
      $('build-search').value = '';
    });
    if (host.childElementCount) {
      const clearAll = node('button', 'Clear filters', 'build-filter-clear');
      ...
      host.append(node('span', 'All eras and piece ranges', 'build-filter-default muted'));
    }
  }

  function paintAtlasSelection() {
    for (const tile of document.querySelectorAll('.build-matrix-cell')) {
      tile.setAttribute('aria-pressed', String(tile.dataset.era === profileBrowse.era && tile.dataset.band === profileBrowse.band));
    }
  }

  function renderBuildMatrix() {
    const section = node('section', null, 'build-matrix');
    section.id = 'build-matrix';
    const heading = node('h3', 'Build counts by your credited pieces');
    heading.id = 'build-matrix-h3';
    section.setAttribute('aria-labelledby', heading.id);
    section.append(heading, node('p', 'Columns group builds by pieces credited to this builder. Select a cell for its largest builds. Photographs above remain global; these counts cover the full profile.', 'muted'));
    const wrap = node('div', null, 'build-matrix-wrap');
    const table = node('table', null, 'build-matrix-table');
    table.append(node('caption', 'Build counts by era and credited pieces. Each cell also states how many builds have photographs.'));
    const head = node('thead');
    const header = node('tr');
    const eraHead = node('th', 'Era');
    eraHead.scope = 'col';
    header.append(eraHead);
    for (const band of buildInventory.bands) {
      const th = node('th', band.label);
      th.scope = 'col';
      header.append(th);
    }
    head.append(header);
    ...
    for (const era of buildInventory.eras) {
      const row = node('tr');
      const eraLabel = node('th', `Era ${era}`);
      eraLabel.scope = 'row';
      row.append(eraLabel);
      for (const band of buildInventory.bands) {
        const td = node('td');
        td.dataset.bandLabel = band.label;
        const cell = buildInventory.cells.get(`${era}:${band.key}`);
        if (cell?.builds) {
          const tile = node('button', null, 'build-matrix-cell');
          tile.type = 'button';
          tile.dataset.era = String(era);
          tile.dataset.band = band.key;
          tile.dataset.density = String(cell.builds < 5 ? 1 : cell.builds < 20 ? 2 : cell.builds < 50 ? 3 : 4);
          tile.setAttribute('aria-pressed', String(profileBrowse.era === String(era) && profileBrowse.band === band.key));
          tile.setAttribute('aria-label', `Era ${era}, ${band.label} credited pieces: ${cell.builds} builds, ${cell.photographed} photographed`);
          tile.append(node('strong', cell.builds.toLocaleString()), node('small', `${cell.photographed} with photos`));
          tile.onclick = () => {
            profileBrowse.era = String(era);
            profileBrowse.band = band.key;
            profileBrowse.query = '';
            profileBrowse.ledgerOpen = false;
            profileBrowse.buildPage = 0;
            selectedBuildKey = null;
            $('build-era').value = String(era);
            $('build-search').value = '';
            refreshBrowse();
            $('build-browse')?.scrollIntoView({block: 'start'});
          };
          ...
          td.append(node('span', '–', 'build-matrix-empty'));
        }
        row.append(td);
      }
      body.append(row);
    ...
    table.append(body);
    wrap.append(table);
    section.append(wrap, node('p', 'Darker cells hold more builds; printed counts remain authoritative. Unmeasured historical credits are kept separate.', 'build-matrix-legend muted'));
    return section;
  }

  function renderFocusedBuild(album) {
    const panel = node('article', null, 'album build-focus');
    panel.dataset.buildKey = album.buildKey;
    panel.setAttribute('aria-label', `${album.label}, era ${album.era}`);
    panel.append(node('h3', album.label));
    const facts = node('p', `Era ${album.era} · ${album.pieces.toLocaleString()} pieces · ${plural(album.photos?.length || 0, 'photograph')}`, 'build-focus-facts');
    panel.append(facts, restOverview(album));
    if (album.photoStatus === 'rejected') panel.append(node('p', WITHHELD_NOTE, 'muted'));
    if (album.attribution) panel.append(node('p', album.attribution, 'muted'));
    const links = node('div', null, 'links');
    if (album.worldUrl) links.append(link('Open in world viewer', album.worldUrl));
    if (album.galleryUrl) links.append(link('Open original gallery', album.galleryUrl));
    if (album.photos?.length) {
      const jump = node('button', 'See photographs above', 'secondary');
      jump.type = 'button';
      jump.onclick = () => {
        const index = workAlbums.findIndex((item) => item.buildKey === album.buildKey);
        if (index < 0) return;
        workIndex = index;
        workPhoto = 0;
        profileBrowse.workPage = Math.floor(index / PAGE_SIZE_ALBUMS);
        renderWorkRail();
        paintWorkStage();
        $('work-stage')?.scrollIntoView({block: 'start'});
      };
      links.append(jump);
    }
    panel.append(links, renderAlbumActions(album, thread.builderKey));
    if (!album.photos?.length) {
      const priority = node('div', null, 'build-focus-priority');
      priority.append(node('span', 'Photography priority'), renderPriorityGroup(album, thread.builderKey));
      panel.append(priority);
    }
    return panel;
  }

  function focusBuild(buildKey, {scroll = false} = {}) {
    const album = browseAlbums.find((item) => item.buildKey === buildKey);
    if (!album) return null;
    selectedBuildKey = buildKey;
    const holder = $('build-focus-host');
    holder.replaceChildren(renderFocusedBuild(album));
    holder.hidden = false;
    for (const row of $('build-explorer').querySelectorAll('[data-build-key]')) {
      if (row.tagName === 'TR') row.setAttribute('aria-selected', String(row.dataset.buildKey === buildKey));
      else row.setAttribute('aria-pressed', String(row.dataset.buildKey === buildKey));
    }
    renderTagChips(externalParticipation);
    if (scroll) {
    ...
    return holder.querySelector('article.album');
  }

  function renderShortlistPick(album) {
    const pick = node('button', null, 'build-shortlist-pick');
    pick.type = 'button';
    pick.dataset.buildKey = album.buildKey;
    pick.setAttribute('aria-pressed', String(album.buildKey === selectedBuildKey));
    const photo = album.photos?.[0];
    if (photo?.thumb) {
      const preview = node('span', null, 'build-shortlist-preview');
      const img = document.createElement('img');
      img.src = photo.thumb;
      img.alt = `Photograph of ${album.label}, era ${album.era}`;
      img.loading = 'lazy';
      img.decoding = 'async';
      img.onerror = () => {
        img.remove();
        preview.classList.add('is-unavailable');
        preview.append(node('span', 'Photograph unavailable'));
      };
      preview.append(img);
      pick.append(preview);
    } else {
      pick.append(node('span', photo ? 'Photograph unavailable' : album.photoStatus === 'rejected' ? 'Photographed; reshoot planned' : 'Not yet photographed', 'build-shortlist-record'));
    }
    const mine = (album.contributors || []).find((entry) => entry?.builderKey === thread.builderKey)?.pieces;
    const copy = node('span', null, 'build-shortlist-copy');
    copy.append(node('strong', album.label),
      node('span', `Era ${album.era} · ${mine == null ? 'unmeasured credit' : `${mine.toLocaleString()} your pieces`} · ${album.pieces.toLocaleString()} total`),
      node('span', photo ? plural(album.photos.length, 'photograph') : album.photoStatus === 'rejected' ? 'Reshoot planned' : 'No photographs yet', 'build-shortlist-status'));
    pick.append(copy);
    pick.onclick = () => focusBuild(album.buildKey, {scroll: true});
    return pick;
  }

  function renderBuildExplorer() {
    const section = node('section', null, 'build-explorer');
    section.id = 'build-explorer';
    section.setAttribute('aria-labelledby', 'build-explorer-h2');
    const heading = node('h2', 'Build explorer');
    heading.id = 'build-explorer-h2';
    heading.tabIndex = -1;
    section.append(heading);
    if (!browseAlbums.length) {
      section.append(node('p', 'No builds match these filters. Remove a filter above or choose another era.', 'muted'));
      const empty = node('div');
      empty.id = 'build-focus-host';
      empty.hidden = true;
      empty.tabIndex = -1;
      empty.setAttribute('role', 'region');
      empty.setAttribute('aria-label', 'Selected build details');
      section.append(empty);
      return section;
    }
    if (!profileBrowse.ledgerOpen) {
      const bandLabel = PROFILE_PIECE_BANDS.find((band) => band.key === profileBrowse.band)?.label;
      const context = profileBrowse.band !== 'all'
        ? `Era ${profileBrowse.era}, ${bandLabel} credited pieces`
        : profileBrowse.era === 'all' ? 'across this profile' : `in era ${profileBrowse.era}`;
      const limit = profileBrowse.era === 'all' && profileBrowse.band === 'all' && !profileBrowse.query ? 6 : 10;
      const picks = profileBrowse.sort === 'guided' ? profileGuidedPicks(browseAlbums, limit) : browseAlbums.slice(0, limit);
      if (selectedBuildKey && !picks.some((album) => album.buildKey === selectedBuildKey)) {
        const selected = browseAlbums.find((album) => album.buildKey === selectedBuildKey);
        if (selected) picks.push(selected);
      }
      const selection = profileBrowse.sort === 'guided' ? 'a mix of photographed highlights and substantial work awaiting photographs' : 'the first builds in your chosen order';
      section.append(node('p', `${plural(browseAlbums.length, 'build')} match ${context}. Showing ${plural(picks.length, 'build')}: ${selection}; use search above or the complete ledger for a particular key.`, 'muted'));
      const list = node('ul', null, 'build-shortlist');
      for (const album of picks) {
        const item = node('li');
        item.append(renderShortlistPick(album));
        list.append(item);
      }
      section.append(list);
      const full = node('button', `View complete ledger (${browseAlbums.length.toLocaleString()} builds)`, 'secondary');
      full.type = 'button';
      full.onclick = () => {
        profileBrowse.ledgerOpen = true;
        profileBrowse.buildPage = 0;
        $('build-explorer').replaceWith(renderBuildExplorer());
        $('build-explorer-h2')?.focus({preventScroll: true});
      };
      section.append(full);
      const focusHost = node('div');
      focusHost.id = 'build-focus-host';
      focusHost.hidden = true;
      focusHost.tabIndex = -1;
      focusHost.setAttribute('role', 'region');
      focusHost.setAttribute('aria-label', 'Selected build details');
      section.append(focusHost);
      if (selectedBuildKey && browseAlbums.some((album) => album.buildKey === selectedBuildKey)) {
        focusHost.replaceChildren(renderFocusedBuild(browseAlbums.find((album) => album.buildKey === selectedBuildKey)));
        focusHost.hidden = false;
      }
      return section;
    }
    const page = profileBrowsePage(browseAlbums, profileBrowse.buildPage, PAGE_SIZE_EXPLORER);
    profileBrowse.buildPage = page.page;
    const close = node('button', 'Back to shortlist', 'secondary');
    close.type = 'button';
    close.onclick = () => {
      profileBrowse.ledgerOpen = false;
      $('build-explorer').replaceWith(renderBuildExplorer());
    ...
    const wrap = node('div', null, 'build-explorer-wrap');
    const table = node('table', null, 'build-explorer-table');
    table.append(node('caption', `${plural(browseAlbums.length, 'build')} match this view; ${plural(page.shown.length, 'build')} shown on this page.`));
    const head = node('thead');
    const labels = ['Build', 'Era', 'Your pieces', 'Total pieces', 'Photographs'];
    ...
    table.append(head);
    const body = node('tbody');
    for (const album of page.shown) {
      const row = node('tr');
      row.dataset.buildKey = album.buildKey;
      row.setAttribute('aria-selected', String(album.buildKey === selectedBuildKey));
      const name = node('td');
      const pick = node('button', album.label, 'build-explorer-pick');
      pick.type = 'button';
      pick.onclick = () => focusBuild(album.buildKey, {scroll: true});
      name.append(pick);
      row.append(name, node('td', String(album.era)));
      const mine = (album.contributors || []).find((item) => item.builderKey === thread.builderKey);
      row.append(node('td', mine?.pieces == null ? 'unmeasured' : mine.pieces.toLocaleString()));
      row.append(node('td', album.pieces.toLocaleString()));
      row.append(node('td', album.photos?.length ? `${album.photos.length} photographed` : 'Not yet photographed'));
      body.append(row);
    }
    table.append(body);
    wrap.append(table);
    section.append(wrap);
    const pager = node('div', null, 'build-pager');
    const previous = node('button', 'Previous', 'secondary');
    const next = node('button', 'Next', 'secondary');
    previous.type = next.type = 'button';
    previous.disabled = page.page === 0;
    next.disabled = page.page >= page.pages - 1;
    const go = (number) => {
      profileBrowse.buildPage = number;
      selectedBuildKey = null;
      $('build-explorer').replaceWith(renderBuildExplorer());
      $('build-explorer').querySelector('.build-page-status')?.focus({preventScroll: true});
    };
    previous.onclick = () => go(page.page - 1);
    next.onclick = () => go(page.page + 1);
    const first = page.page * PAGE_SIZE_EXPLORER + 1;
    const status = node('span', `${first.toLocaleString()}–${(first + page.shown.length - 1).toLocaleString()} of ${page.total.toLocaleString()} · page ${page.page + 1} of ${page.pages}`, 'build-page-status');
    status.tabIndex = -1;
    status.setAttribute('aria-live', 'polite');
    pager.append(previous, status, next);
    const jumpLabel = node('label', 'Go to page ', 'build-page-jump');
    const jump = node('input');
    jump.type = 'number';
    jump.min = '1';
    jump.max = String(page.pages);
    jump.value = String(page.page + 1);
    jump.setAttribute('aria-label', `Go to page, 1 through ${page.pages}`);
    jump.onchange = () => go(Math.max(0, Math.min(page.pages - 1, Number(jump.value || 1) - 1)));
    jumpLabel.append(jump);
    pager.append(jumpLabel);
    ...
    focusHost.setAttribute('aria-label', 'Selected build details');
    section.append(focusHost);
    if (selectedBuildKey && page.shown.some((album) => album.buildKey === selectedBuildKey)) {
      focusHost.replaceChildren(renderFocusedBuild(page.shown.find((album) => album.buildKey === selectedBuildKey)));
      focusHost.hidden = false;
    }
    return section;
  }
  ...
  function refreshBrowse() {
    ...
    browseAlbums = profileBrowseRows(thread, profileBrowse);
    const shot = browseAlbums.filter((album) => album.photos?.length).length;
    const unshot = browseAlbums.length - shot;
    $('build-browse-summary').textContent = thread.albums === 0
      ...
      : `${plural(browseAlbums.length, 'build')} of ${plural(thread.albums, 'build')} match · ${plural(shot, 'photographed build')} · ${plural(unshot, 'unphotographed build')}.`;
    if (selectedBuildKey && !browseAlbums.some((album) => album.buildKey === selectedBuildKey)) selectedBuildKey = null;
    paintAtlasSelection();
    paintEraSelection();
    paintBrowseFilters();
    $('build-explorer').replaceWith(renderBuildExplorer());
    $('kin-beside').replaceWith(renderKinshipEmbed());
    // Pair filters belong to the relationship, not to the build inventory's era facet.
    renderTagChips(externalParticipation);
  }
  ...
  function renderThread() {
    ...
    eraAtlas = profileEraAtlas(thread);
    buildInventory = profileBuildInventory(thread);
    coBuilderSummaries = new Map([['all', profileCoBuilderSummary(thread)]]);
    for (const record of eraAtlas) coBuilderSummaries.set(String(record.era), profileCoBuilderSummary(thread, String(record.era)));
    browseAlbums = profileBrowseRows(thread, profileBrowse);
    ...
    const hasPhotographs = thread.eras.some((block) => block.albums.some((album) => album.photos?.length));
    if (hasPhotographs) {
      ...
      workAlbums = [];
      if (thread.albums) $('content').append(node('p', 'No photographs have been published for these builds yet. The saved-piece record is ready to explore below.', 'work-empty muted'));
    }
    if (thread.albums) $('content').append(renderEraOverview());
    $('content').append(renderBrowseToolbar());
    if (thread.albums) paintBrowseFilters();
    $('build-browse-summary').textContent = thread.albums === 0
      ? 'No substantial build albums meet the 20-piece saved-credit cutoff yet. This creator remains searchable; smaller credits and historical context are still in the archive.'
      : `${plural(browseAlbums.length, 'build')} of ${plural(thread.albums, 'build')} match · ${plural(shotCount, 'photographed build')} · ${plural(unshotCount, 'unphotographed build')}.`;
    ...
    // The work remains the global photographic opening; the compact era guide and
    // adjacent browser focus only the explorer below it.
    $('content').append(renderBuildExplorer());

    $('content').append(renderKinshipEmbed());
    renderThreadNotes();
