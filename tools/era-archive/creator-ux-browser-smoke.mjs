...
try {
  ...
  results.profile = await evaluate("({heading:document.querySelector('#title').textContent,matrixCount:[...document.querySelectorAll('.build-matrix-cell strong')].reduce((n,e)=>n+Number(e.textContent.replace(/,/g,'')),0),shortlist:document.querySelectorAll('.build-shortlist li').length,ledgerRows:document.querySelectorAll('#build-explorer tbody tr').length,matrixWidth:Math.round(document.querySelector('.build-matrix-table').getBoundingClientRect().width),matrixHeight:Math.round(document.querySelector('.build-matrix-table').getBoundingClientRect().height),cellHeight:Math.round(document.querySelector('.build-matrix-cell').getBoundingClientRect().height)})");
  if (results.profile.matrixCount !== 547 || results.profile.shortlist > 5 || results.profile.ledgerRows !== 0 || results.profile.matrixWidth > 900 || results.profile.matrixHeight > 760 || results.profile.cellHeight > 48) throw Error(`Default inventory is still unwieldy: ${JSON.stringify(results.profile)}`);
  ...
  await cdp('Page.navigate', {url: new URL(`${builderKey}/`, base).href});
  await waitFor("!!document.querySelector('#build-matrix tbody tr')");
  results.profileMobile = await evaluate("({viewport:innerWidth,scrollWidth:document.documentElement.scrollWidth,columns:getComputedStyle(document.querySelector('#build-matrix tbody tr')).gridTemplateColumns.split(' ').length,cellHeight:Math.round(document.querySelector('.build-matrix-cell').getBoundingClientRect().height)})");
  if (results.profileMobile.columns !== 2 || results.profileMobile.scrollWidth > results.profileMobile.viewport + 2 || results.profileMobile.cellHeight > 48) throw Error(`Mobile inventory is unbounded: ${JSON.stringify(results.profileMobile)}`);
  await evaluate("document.querySelector('#build-matrix').scrollIntoView({block:'start'})");
  await screenshot('profile-matrix-mobile');
  await cdp('Emulation.clearDeviceMetricsOverride');
  await cdp('Page.navigate', {url: new URL(`${builderKey}/`, base).href});
  await waitFor("!!document.querySelector('#build-matrix .build-matrix-cell')");
