...
public class ZPackage implements AutoCloseable {
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ZPackage.class);
	...
	public int readNumItems(int worldVersion) {
		...
		int num = Byte.toUnsignedInt(readByte());
		if ((num & 128) != 0) {
			num = ((num & 127) << 8) | Byte.toUnsignedInt(readByte());
		}
		return num;
	}
